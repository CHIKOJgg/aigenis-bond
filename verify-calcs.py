...
from desk.duration import macaulay_duration, modified_duration, convexity
...
check("zero-coupon ytm matches (100/85)^(1/3)-1", y is not None and abs(y - expected) < 0.05, f"got {y}, expected {expected}")

# Price 0 / negative / no maturity -> None
check("ytm guards bad inputs",
      ytm_from_price(0.0, 10.0, 2, date(2030, 1, 1)) is None and
      ytm_from_price(-5.0, 10.0, 2, date(2030, 1, 1)) is None and
      ytm_from_price(100.0, 10.0, 2, date(2020, 1, 1)) is None)
...
check("absolute price 10039.58 on 10000 nominal -> 100.396%", abs((to_price_pct(10039.58, 10000.0) or 0) - 100.3958) < 1e-6)
check("percent price 99.5 passes through", abs((to_price_pct(99.5, 1000.0) or 0) - 99.5) < 1e-9)
check("percent 100.0 on 200 nominal stays 100.0 (regression: was 50%)", abs((to_price_pct(100.0, 200.0) or 0) - 100.0) < 1e-9)
check("percent 100.0 on 50 nominal stays 100.0 (regression: was 200%)", abs((to_price_pct(100.0, 50.0) or 0) - 100.0) < 1e-9)
...
d = macaulay_duration(
    nominal=Decimal(1000), coupon_rate_pct=0.0, coupon_frequency=1, ytm_pct=5.57,
    maturity=date(2029, 8, 8), ref=date(2026, 8, 8),
)
...
d = macaulay_duration(
    nominal=Decimal(1000), coupon_rate_pct=10.0, coupon_frequency=2, ytm_pct=10.0,
    maturity=date(2036, 8, 8), ref=date(2026, 8, 8),
)
...
acc = accrued_interest(
    coupon_rate_pct=10.0, coupon_frequency=2,
    issue_date=date(2026, 2, 10), maturity_date=date(2036, 2, 10), asof=date(2026, 8, 11),
)
check("day after coupon -> ~0", acc is not None and acc < 0.1, f"got {acc}")
acc = accrued_interest(
    coupon_rate_pct=10.0, coupon_frequency=2,
    issue_date=date(2026, 2, 10), maturity_date=date(2036, 2, 10), asof=date(2027, 2, 9),
)
...
bs = score_bond(
    internal_id="t1", yield_to_maturity=14.2, currency="BYN",
    maturity_date=date(2028, 6, 15), status="active", issuer="Минфин РБ",
    price=98.5, nominal=Decimal("100"), coupon_rate=12.5, market="bcse",
)
bd = bs.breakdown
check("score in [0,100]", 0 <= bs.score <= 100, f"score={bs.score}")
check("tier in S/A/B/C/D", bs.tier in ("S", "A", "B", "C", "D"), f"tier={bs.tier}")
reward_comps = [bd.yield_component, bd.currency_component, bd.duration_component,
                bd.liquidity_component, bd.metal_component, bd.credit_risk_component,
                bd.inflation_component, bd.coupon_component,
                bd.historical_volatility_component, bd.peer_relative_component]
check("reward_subtotal = sum of positive comps (excl. volatility)",
...
check("risk_subtotal = sum of |negative comps|",
...
check("efficiency = reward/(reward+risk+1)*15 (2dp rounding)",
      abs(bd.efficiency_ratio - expected_eff) < 0.005,
      f"eff={bd.efficiency_ratio}, expected={expected_eff}")
check("score = reward - risk (all comps net)",
      abs(bs.score - (bd.reward_subtotal - bd.risk_subtotal)) < 1e-6,
      f"score={bs.score} net={bd.reward_subtotal - bd.risk_subtotal}")
check("high YTM + low risk -> attractive status",
      bs.tier in ("S", "A", "B") or bs.score >= 70, f"tier={bs.tier} score={bs.score}")

bs2 = score_bond(
    internal_id="t2", yield_to_maturity=3.0, currency="RUB",
    maturity_date=date(2035, 1, 1), status="active", issuer="ОФЗ",
    price=90.0, nominal=Decimal("100"), coupon_rate=4.0, market="moex",
)
check("low yield -> not attractive", bs2.tier in ("C", "D") or bs2.score < 60,
