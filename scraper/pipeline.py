"""Бизнес-логика сбора данных (run-once + backfill)."""
...
from scraper.errors import HistoryUnavailable, NotFoundError, ParseError, TransientError
...
async def collect_listing(client: AigenisClient, currencies: Iterable[str]) -> list[str]:
    """Собрать internal_id по всем валютам. Возвращает уникальный список."""
    seen: set[str] = set()
...
async def collect_details(
    ...
    *,
    ...
    """Скачать и сохранить детали. Возвращает (успешно, ошибок)."""
    ok, err = 0, 0
    ...
    async def _one(iid: str) -> tuple[str, str]:
        async with sem:
            try:
                ...
                logger.exception("detail_unexpected", internal_id=iid, error=str(e))
                return iid, "error"
...
def _delisted_placeholder(internal_id: str, currency: str = "unknown"):
...
async def backfill_history(
    ...
    *,
    ...
    """Догрузить историю за последние `days` дней."""
    ...
    async def _one(iid: str) -> None:
        ...
        async with sem:
            ...
            try:
                ...
                logger.info("history_unavailable", internal_id=iid)
                return
            except NotFoundError:
                return
            except (TransientError, ParseError) as e:
                ...
                logger.exception("history_unexpected", internal_id=iid, error=str(e))
                err += 1
                return
...
async def enrich_from_xlsx(xlsx_data: XlsxParseResult | None = None) -> dict[str, int]:
    """Обогатить данные облигаций из XLSX-файлов."""
    ...
    async with session_scope() as session:
        ...
        for bond_orm in all_bonds:
            ...
            # Enrich coupon_schedule from coupon_periods
        ...
        if xlsx_data.daily_accruals:
            ...
            if remapped:
                accruals_written = await repositories.history.upsert_accruals_batch(session, remapped)
