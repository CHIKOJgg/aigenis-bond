...
def test_bull_usd_strengthens_byn_value():
    res = run_scenario("Bull USD", current_usd_byn=Decimal("3.0"),
                       usd_share=0.6, byn_share=0.4)
...
def test_neutral_no_change():
    res = run_scenario("Neutral", current_usd_byn=Decimal("3.0"),
                       usd_share=0.5, byn_share=0.5)
...
def test_bull_byn_weakens_usd_value():
    res = run_scenario("Bull BYN", current_usd_byn=Decimal("3.0"),
                       usd_share=0.5, byn_share=0.5)
...
def test_stress_reduces_value():
    res = run_scenario("Stress", current_usd_byn=Decimal("3.0"),
                       usd_share=0.8, byn_share=0.2)
...
def test_metals_and_eur_impact():
    res = run_scenario("Bull USD", current_usd_byn=Decimal("3.0"),
                       usd_share=0.0, byn_share=0.0, metals_share=1.0, eur_share=1.0)
...
def test_run_all_scenarios_four():
    results = run_all_scenarios(current_usd_byn=Decimal("3.0"),
                                usd_share=0.5, byn_share=0.5)
