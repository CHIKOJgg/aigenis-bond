...
def _bond(
    internal_id, ytm=10.0, price=100.0, currency="BYN", is_gov=False, maturity=date(2028, 1, 1)
):
...
def test_parallel_up_shock_loses_value():
    ...
    res = run_stress(
        scn, [(_bond("A"), Decimal("1000"))], base_currency="BYN", asof=date(2024, 1, 1)
    )
    assert res.pnl < 0
...
def test_credit_shock_spares_government():
    ...
    res = run_stress(
        scn,
        [(_bond("GOV", is_gov=True), Decimal("1000"))],
        base_currency="BYN",
        asof=date(2024, 1, 1),
    )
...
def test_credit_shock_hits_corporate():
    ...
    res = run_stress(
        scn,
        [(_bond("CORP", is_gov=False), Decimal("1000"))],
        base_currency="BYN",
        asof=date(2024, 1, 1),
    )
    assert res.pnl < 0
...
def test_fx_shock_usd_gains_against_byn_base():
    ...
    res = run_stress(
        scn,
        [(_bond("USD", currency="USD"), Decimal("1000"))],
        base_currency="BYN",
        asof=date(2024, 1, 1),
    )
...
def test_fx_shock_byn_base_bond_unchanged():
    ...
    res = run_stress(
        scn,
        [(_bond("BYN", currency="BYN"), Decimal("1000"))],
        base_currency="BYN",
        asof=date(2024, 1, 1),
    )
    assert res.pnl == Decimal("0.00")
...
def test_run_stress_values_expired_bond_via_duration_fallback():
    ...
    res = run_stress(
        scn,
        [(_bond("OLD", maturity=date(2020, 1, 1)), Decimal("1000"))],
        base_currency="BYN",
        asof=date(2024, 1, 1),
    )
