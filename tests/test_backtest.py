...
def test_score_conservative_formula():
    ...
    assert (
        _score_bond_for_strategy(b, "Conservative", price=Decimal("100"), ytm=Decimal("10")) == 3.0
    )
...
def test_score_conservative_discount_bonus():
    ...
    # 3.0 + (100-90)*0.01 = 3.1
    assert (
        _score_bond_for_strategy(b, "Conservative", price=Decimal("90"), ytm=Decimal("10")) == 3.1
    )
...
def test_score_carry_trade_uses_coupon():
    ...
    assert (
        _score_bond_for_strategy(b, "Carry Trade", price=Decimal("100"), ytm=Decimal("10")) == 11.2
    )
...
def test_score_dollarization_usd_bonus():
    ...
    usd_score = _score_bond_for_strategy(
        usd, "Dollarization", price=Decimal("100"), ytm=Decimal("10")
    )
    byn_score = _score_bond_for_strategy(
        byn, "Dollarization", price=Decimal("100"), ytm=Decimal("10")
    )
...
def test_build_price_rows_sorts_and_filters_none():
    hist = {
        "A": _hist(
            "A",
            [(date(2024, 1, 2), 101, 9), (date(2024, 1, 1), 100, 10), (date(2024, 1, 3), None, 8)],
        ),
    }
...
def test_snapshot_on_or_before_returns_last_known():
    hist = {"A": _hist("A", [(date(2024, 1, 1), 100, 10), (date(2024, 1, 5), 102, 9)])}
    rows = _build_price_rows(hist)
